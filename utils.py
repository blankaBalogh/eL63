...
def plot_3d_timeseries(z_Bz, i0=0, cbar=False, cmap="", save=False, sdir="", 
        title="Lorenz", scatter=False, quiver=False, sformat="eps", scaled=True) :
    """
    Plots 3D timeseries.
...
# -- Getting indexes of instable initial conditions.
def divergence(xML, mini, maxi) :
    '''
    Returns list of diverging intial condition indexes. 
    '''
    if np.isnan(np.sum(xML)) :
        list_divergence = ['all']
        return(list_divergence)
    else :
        minlist = list(np.unique(np.where((np.nanmax(xML, axis=0))>maxi)[0]))
        maxlist = list(np.unique(np.where((np.nanmin(xML, axis=0))<mini)[0]))
        list_divergence = list(dict.fromkeys(minlist + maxlist))
        return(list_divergence)


# -- Diagnostic of stability.
def is_stable(xML, mini, maxi, print_nans=False) :
    '''
    Print whether "trajectories" is diverging or not regarding (mini, maxi) values.
    '''   
    if ((np.nanmax(xML, axis=(0,1)) > maxi).astype(np.int)).sum()==0 :
        if ((np.nanmax(xML, axis=(0,1))< mini).astype(np.int)).sum()==0 :
            return(True)
        else :
            return(False)
    else :
...
# -- Diagnostic of fixed points.
def diag_fixed_points(xML, threshold=0.5) :
    '''
    Returns 'True' if a neural network predicted orbit collapses into a fixed point,
    regarding a threshold value.  
    '''
    std = np.std(xML, axis=0)
    for i in range(xML.shape[1]) :
        if std[i] < threshold :
            return True
    return False
